...
async function loadArtworks() {
    try {
        // Fetch artworks from Firestore (global collection)
        const snapshot = await getDocs(collection(db, "artworks"));

        const allArtworks = [];
        snapshot.forEach(docSnap => {
            allArtworks.push(docSnap.data());
        });

        // (Optional) merge with static demo data if you still want defaults if not add "//" before the next 2 lines
        const combinedArtworks = [...artworkData, ...allArtworks];
        currentArtworks = combinedArtworks;

        // if you only want the data from the db remove the "//" on the next line
